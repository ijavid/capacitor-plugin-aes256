...
public class AES256Plugin: CAPPlugin {
    ...
    private var defaultIv = ""

    override public func load() {
        defaultKey = getConfig().getString("secureKey") ?? ""
        defaultIv = getConfig().getString("secureIv") ?? ""
    }
    ...
    @objc func generateSecureKey(_ call: CAPPluginCall) {
        ...
        let secureKey = PBKDF2.pbkdf2(hash:CCPBKDFAlgorithm(kCCPRFHmacAlgSHA1), password:password, salt:AES256CBC.generateSalt(), keyByteCount:AES256Plugin.SECURE_KEY_LENGTH, rounds:AES256Plugin.PBKDF2_ITERATION_COUNT) ?? "";
        call.resolve([
            "response": secureKey
        ])
    }
    ...
    @objc func generateSecureIv(_ call: CAPPluginCall) {
        ...
        let secureIV = PBKDF2.pbkdf2(hash:CCPBKDFAlgorithm(kCCPRFHmacAlgSHA1), password:password, salt:AES256CBC.generateSalt(), keyByteCount:AES256Plugin.SECURE_IV_LENGTH, rounds:AES256Plugin.PBKDF2_ITERATION_COUNT) ?? "";
        call.resolve([
            "response": secureIV
        ])
    }
